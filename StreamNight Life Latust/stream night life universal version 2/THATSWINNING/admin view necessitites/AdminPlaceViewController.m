...
@interface AdminPlaceViewController ()

@end

@implementation AdminPlaceViewController
...
-(IBAction)onClickApprove_Button:(id)sender {
    ...
    if (btn.selected) {
        ...
        NSString *strurl;
        strurl=[NSString stringWithFormat:@"http://www.thatswinning.com/traker/?action=EditPlaceStatus&place_id=%@&place_status=%@",self.place_id,statusflag];
...
-(IBAction)onClickReport_Button:(id)sender {
    ...
    reportViewController.idstr = self.place_id;
    reportViewController.adminFlag = YES;

}
-(IBAction)onClickDelete_Button:(id)sender {
    NSString *strurl;
    strurl=[NSString stringWithFormat:@"http://www.thatswinning.com/traker/?action=DeletePlace&place_id=%@",self.place_id];
    ...
    if ([[recievedData objectForKey:@"message"] isEqualToString:@"Successful"]) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Shared Place Deleted" message:nil delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
        [alert show];
        [self.navigationController popViewControllerAnimated:YES];
    }
}
